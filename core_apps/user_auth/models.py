...
class User(AbstractUser):
    class SecurityQuestion(models.TextChoices):
        ...
        FAVOURITE_COLOR = (
            "favourite_color",
            _("What is your favourite color?"),
        )
        BIRTH_CITY = (
            "birth_city",
    ...
    id_no = models.IntegerField(_("ID Number"), unique=True)
    ...
    def handle_failed_login_attempts(self) -> None:
        ...
        if self.failed_login_attempt >= settings.LOGIN_ATTEMPTS:
    ...
    def is_locked_out(self) -> bool:
        if self.account_status == self.AccountStatus.LOCKED:
            if (
                ...
                return False  # Lock has expired, but don't change anything yet
            return True  # Still locked
        return False

    def maybe_unlock_account(self) -> None:
        if self.account_status == self.AccountStatus.LOCKED:
            if (
                self.last_failed_login
                and (timezone.now() - self.last_failed_login)
    ...
    # user.maybe_unlock_account()  # Try to unlock if time has expired

    # if user.is_locked_out():
    #     raise ValidationError(_("Your account is temporarily locked. Please try again later."))
